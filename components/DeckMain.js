...
class DeckMain extends Component {
  ...
  renderItem = ({item})  => (
    <TouchableOpacity
      ...
      <View>
        <Card
          title={item.title}>
          <Text style={{alignSelf: 'center'}}>{`${item.questions.length} card`}</Text>
        </Card>
      </View>
    </TouchableOpacity>
  )
  ...
  render() {
    ...
    return (
      ...
      {decks.length > 0
        ? <View style={{flex:1, alignSelf: 'stretch'}}><FlatList
            data={decks}
            renderItem={this.renderItem}
            keyExtractor={this._keyExtractor}/></View>
        : <View style={{alignItems: 'center', justifyContent: 'center',flex: 1}}><Text>{`Create a deck to get started!`}</Text></View>
    }
      </View>
    )
  }
